...
if ( ! function_exists( 'muffle_posted_by' ) ) :
	/**
...
if ( ! function_exists( 'muffle_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function muffle_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'muffle' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'muffle' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'muffle' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'muffle' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'muffle' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'muffle' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'muffle_entry_meta_footer' ) ) :
		/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 ...
	 * @return void   
	 */
	function muffle_entry_meta_footer() {
		$readmore_butotn_text = muffle_options('muffle_read_more_text_button', 'Read More');
		$post = get_the_ID();
	?>
...
if ( ! function_exists( 'muffle_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function muffle_post_thumbnail() {
		
$thum_img = is_active_sidebar ('sidebar-1') ? 'muffle-blog' : 'full';
		...
		if ( is_singular() ) :
			...
			<div class="post-thumbnail">
				<?php the_post_thumbnail( 'muffle-blog-page' ); ?>
			</div><!-- .post-thumbnail -->
...
function muffle_is_comment_by_post_author( $comment = null ) {
 ...
 */
 ...
 if(!function_exists('muffle_logo')) {
	...
	*/
		...
		function muffle_logo () {
			...
			if(class_exists( 'Redux' ) ) {
				...
				$logo = muffle_options('muffle_logo'); 
				$logo_sticky = muffle_options('muffle_sticky_logo'); 
				$logo_ratina = muffle_options('muffle_retina_logo');
				$logo_ratina_sticky = muffle_options('muffle_retina_sticky_logo');
  ...
  * display tag 
  */
 if(!function_exists('muffle_single_page_tag'))  {
	 function muffle_single_page_tag() {
		 if(has_tag() ) :
		 ?>
		 <div class="tagcloud">
            <?php the_tags(null, ''); ?>
...
if(!function_exists('muffle_about_author')) {
	
	function muffle_about_author() {
		global $post;
		$bio =  get_the_author_meta( 'description',$post->post_author);
		if(!empty($bio)):
      ?>
	  <div class="about-autheor">
		<h3 class="comment-reply-title"><?php echo esc_html__( 'About the Author', 'muffle' ); ?></h3>
